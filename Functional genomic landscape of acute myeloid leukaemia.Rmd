...
# 3. Associação dos pacientes às amostras e ao estado de sobrevivência
...
# 4. Formatação dos IDs

## 4.1. Formatação dos títulos das colunas
...
# 5. Remoção de amostras sem dados de expressão
...
# 6. Remoção dos metadados do RNA seq
...
# 7. Preparação da matriz de contagem para edgeR
...
# 8. Preparação do grupo de amostras baseado no estado de sobrevivência
...
# 9. Criação do objeto DGEList para executar a análise de expressão diferencial com edgeR
...
# 10. Armazenmento dos resultados
...
## 11.2. Carregar os conjuntos de genes
...
## 12.2. MDS plot (equivalente ao PCA no edgeR)
...
## 12.3. BCV plot (dispersão biológica)

```{r}
plotBCV(dge, main = "Biological Coefficient of Variation")
```

## 12.4. QLF plot (dispersão ajustada)
